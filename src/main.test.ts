...
describe("Enhanced Error Hierarchy - Basic Error Creation", () => {
  ...
  test("should capture stack trace when requested", () => {
    const StackError = createCustomError<{ code: number }>(
      "StackError", 
      ["code"]
    );
    ...
    assert.ok(error.stack.includes("StackError: An error with stack trace"), 
...
describe("Enhanced Error Hierarchy - Edge Cases and Special Behaviors", () => {
  ...
  test("should check type with checkInstance correctly", () => {
    const TypedError = createCustomError<{ value: number }>(
      "TypedError", 
      ["value"]
    );
  ...
  test("should handle complex inheritance with multiple levels", () => {
    ...
    const Level1Error = createCustomError<{ level1: string }>(
      "Level1Error", 
      ["level1"]
    );
