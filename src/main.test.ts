...
describe("Basic Error Creation", () => {
	...
	test("should capture stack trace when requested", () => {
		const StackError = createCustomError<{ code: number }>("StackError", [
			"code",
		]);
...
describe("Error Hierarchies", () => {
	...
	test("should create a three-level error hierarchy with context at each level", () => {
		...
		assert.deepStrictEqual(hierarchy[0].inheritanceChain, [
			"AppError",
			"DatabaseError",
		]);
...
describe("Advanced Usage Patterns", () => {
	...
	test("should support error factory functions", () => {
		...
		assert.equal(
			error.message,
			"User API error (404): Failed to fetch user 123",
		);
		assert.equal(error.statusCode, 404);
	...
	test("should handle deeply nested context objects", () => {
		...
		assert.equal(
			error.config.database.connection.credentials.username,
			"app_user",
		);
...
describe("Edge Cases and Special Behaviors", () => {
	...
	test("should check type with checkInstance correctly", () => {
		const TypedError = createCustomError<{ value: number }>("TypedError", [
			"value",
		]);
	...
	test("should handle complex inheritance with multiple levels", () => {
		...
		const Level1Error = createCustomError<{ level1: string }>("Level1Error", [
			"level1",
		]);

		const Level2Error = createCustomError<
			{ level2: string },
			typeof Level1Error
		>("Level2Error", ["level2"], Level1Error);

		const Level3Error = createCustomError<
			{ level3: string },
			typeof Level2Error
		>("Level3Error", ["level3"], Level2Error);

		const Level4Error = createCustomError<
			{ level4: string },
			typeof Level3Error
		>("Level4Error", ["level4"], Level3Error);
